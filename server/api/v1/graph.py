...
def disease_detail(name: str, _: CurrentUser = Depends(require_roles("user", "admin", "doctor"))):
    """获取疾病详情"""
...
@router.get("/full")
def full_graph(_: CurrentUser = Depends(require_roles("admin"))):
    """获取完整知识图谱（管理员可视化）"""
...
@router.get("/subgraph")
def entity_subgraph(entity: str, _: CurrentUser = Depends(require_roles("admin"))):
    """获取实体子图（管理员可视化）"""
...
@router.get("/search")
def search_entities(keyword: str = Query(..., min_length=1), _: CurrentUser = Depends(require_roles("admin"))):
    """搜索图谱实体"""
