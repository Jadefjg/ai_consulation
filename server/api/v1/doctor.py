...
def delete_doctor(
    ...
    # 清理人工问诊及回复
    consult_ids = [
        c.id for c in db.query(DoctorConsult).filter(DoctorConsult.doctor_id == doctor_id).all()
    ]
    if consult_ids:
        db.query(DoctorReply).filter(DoctorReply.consult_id.in_(consult_ids)).delete(
            synchronize_session=False
        )
        db.query(DoctorConsult).filter(DoctorConsult.doctor_id == doctor_id).delete(
            synchronize_session=False
        )
