...
def init_graph():
    """初始化Neo4j医疗知识图谱（已有数据时跳过，便于重复部署）"""
    driver = GraphDatabase.driver(settings.neo4j_uri, auth=(settings.neo4j_user, settings.neo4j_password))
    with driver.session() as session:
        existing = session.run("MATCH (d:Disease) RETURN count(d) AS cnt").single()
        disease_count = existing["cnt"] if existing else 0
        if disease_count > 0:
            print(f"知识图谱已存在 {disease_count} 个疾病节点，跳过初始化")
        else:
            _import_graph_data(session)
    driver.close()


def _import_graph_data(session):
    """写入约束与疾病知识数据"""
    constraints = [
